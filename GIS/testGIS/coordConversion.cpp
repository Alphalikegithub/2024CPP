...
GeographicPoint cartesianToGeographic(const CartesianPoint& point) {
    ...
    double theta = atan2(point.z * a, rho * b);
    ...
    double lat = atan2(point.z + e2 * b * pow(sin(theta), 3), rho - e2 * a * pow(cos(theta), 3));
    //double lat = atan2(point.z, sqrt(point.x * point.x + point.y * point.y));
